...
export default function DoahlaStore() {
  ...
  const handlePayment = () => {
    ...
    window.location.href = `upi://pay?pa=${myUPI}&pn=Doahla%20Store&am=${selectedPack.p}&cu=INR&tn=MLBB%20Order`;
    setTimeout(() => {
    ...
    <div className={`min-h-screen transition-colors duration-300 ${darkMode ? 'bg-zinc-900 text-pink-500' : 'bg-[#fff0f5] text-black'}`}>
      ...
      <div className="bg-pink-500 text-white py-1 overflow-hidden whitespace-nowrap border-b border-pink-600">
        <div className="animate-marquee inline-block text-[10px] font-bold uppercase tracking-widest">
          FOLLOW OUR INSTAGRAM PAGE @DOAHLAOFFICIAL_STORE — JOIN OUR WHATSAPP COMMUNITY FOR UPDATES —&nbsp;
        </div>
        <div className="animate-marquee inline-block text-[10px] font-bold uppercase tracking-widest">
          FOLLOW OUR INSTAGRAM PAGE @DOAHLAOFFICIAL_STORE — JOIN OUR WHATSAPP COMMUNITY FOR UPDATES —&nbsp;
        </div>
      ...
      <div className="p-6 flex flex-col items-center relative">
        <button onClick={() => setDarkMode(!darkMode)} className={`absolute right-6 top-6 p-2 rounded-full border-2 border-pink-500 shadow-md ${darkMode ? 'bg-zinc-800' : 'bg-white'}`}>
          {darkMode ? '☀️' : '🌙'}
        </button>
        <h1 className="text-3xl font-black italic mt-4 text-center">DOAHLA OFFICIAL STORE</h1>
      </div>

      <main className="max-w-xl mx-auto px-4 pb-32">
        {activeTab === 'HOME' && !selectedGame && (
          <div className={`p-6 rounded-[2.5rem] border-2 border-pink-500 ${!darkMode ? 'bg-white/50' : 'bg-black/20'}`}>
            <h2 className="text-[10px] font-bold mb-6 uppercase tracking-widest text-center">Select Game</h2>
            <div className="grid grid-cols-2 gap-6">
              
              {/* MLBB - ROUNDED SQUARE */}
              <div onClick={() => setSelectedGame('MLBB')} className="cursor-pointer border-2 border-pink-500 rounded-[2rem] overflow-hidden aspect-square bg-black shadow-lg hover:scale-105 transition-transform">
                <img src="/mlbb.jpg" className="w-full h-full object-cover" alt="MLBB" />
              </div>

              {/* COC - BLACK & WHITE */}
              <div className="relative border-2 border-pink-500/30 rounded-[2rem] overflow-hidden aspect-square bg-black opacity-80">
                <img src="/coc.jpg" className="w-full h-full object-cover grayscale opacity-40" alt="COC" />
                <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-pink-500 text-white text-[8px] font-black px-3 py-1 rounded-full shadow-lg">COMING SOON</span>
              </div>

              {/* BGMI - BLACK & WHITE */}
              <div className="relative border-2 border-pink-500/30 rounded-[2rem] overflow-hidden aspect-square bg-black opacity-80">
                <img src="/bgmi.jpg" className="w-full h-full object-cover grayscale opacity-40" alt="BGMI" />
                <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-pink-500 text-white text-[8px] font-black px-3 py-1 rounded-full shadow-lg">COMING SOON</span>
              </div>

              {/* CODM - BLACK & WHITE */}
              <div className="relative border-2 border-pink-500/30 rounded-[2rem] overflow-hidden aspect-square bg-black opacity-80">
                <img src="/codm.jpg" className="w-full h-full object-cover grayscale opacity-40" alt="CODM" />
                <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-pink-500 text-white text-[8px] font-black px-3 py-1 rounded-full shadow-lg">COMING SOON</span>
              </div>
        ...
        {/* REST OF YOUR WORKING UI (MLBB VIEW, SUPPORT, POLICIES) STAY THE SAME */}
        {selectedGame === 'MLBB' && activeTab === 'HOME' && (
          <div className="space-y-6 animate-in fade-in duration-500">
            ...
            {Object.keys(packs).map(cat => (
              <div key={cat}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {packs[cat].map((p: any) => (
                    <button key={p.n} onClick={() => setSelectedPack(p)} className={`p-3 rounded-xl border-2 transition-all text-left ${selectedPack?.n === p.n ? 'bg-pink-500 text-white border-white' : 'border-pink-500/20'}`}>
            ...
            <button disabled={!selectedPack || !playerID || isProcessing} onClick={handlePayment} className="w-full bg-pink-500 text-white h-16 rounded-2xl font-black italic text-lg shadow-xl active:scale-95 disabled:opacity-20 transition-all">
              {isProcessing ? 'PROCESSING...' : 'PROCEED TO PAY'}
            </button>
          </div>
        )}

        {/* ... SUPPORT & POLICIES VIEWS ... */}
        {activeTab === 'SUPPORT' && (
          <div className="space-y-6">
            ...
            <div className={`p-6 rounded-3xl border-2 border-pink-500 space-y-4 ${darkMode ? 'bg-black' : 'bg-white'}`}>
              <div><p className="text-[10px] font-bold text-pink-500">WHATSAPP SUPPORT</p><p className="font-bold">7085340215 / 70057 95669</p></div>
              <div><p className="text-[10px] font-bold text-pink-500">GMAIL SUPPORT</p><p className="font-bold">evyyzhimomi@gamil.com</p></div>
              <div><p className="text-[10px] font-bold text-pink-500">INSTAGRAM</p><p className="font-bold">@Doahlaofficial_store</p></div>
        ...
        {activeTab === 'POLICIES' && (
          <div className="space-y-4">
            ...
            <div className={`p-6 rounded-3xl border-2 border-pink-500 space-y-4 text-xs font-bold leading-relaxed ${darkMode ? 'bg-black' : 'bg-white'}`}>
              ...
              <p>3. Please refrain from spamming WhatsApp DMs if your order hasn't arrived instantly.</p>
      ...
      <nav className={`fixed bottom-0 left-0 right-0 p-4 border-t border-pink-500/20 flex justify-around items-center z-50 ${darkMode ? 'bg-black/90' : 'bg-white/90'} backdrop-blur-md`}>
        {['HOME', 'SUPPORT', 'POLICIES'].map(tab => (
          <button key={tab} onClick={() => {setActiveTab(tab); setSelectedGame(null);}} className={`text-[10px] font-black tracking-widest transition-all ${activeTab === tab ? 'text-pink-500 scale-110 underline decoration-2 underline-offset-4' : 'text-gray-500'}`}>
            {tab}
          </button>
        ))}
      </nav>
